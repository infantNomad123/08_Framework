...
function getTotalCount(){ //함수 정의
    ...
    //비동기로 서버(DB)에서 전체 Todo 개수를 조회 하는 
    //fetch() API 코드 작성
    // (fetch : 가지고 오다)
    ...
    //두 번째 then의 매개 변수 
    // == 첫 번쨰 then 에서 반환된 Promise 객체의 promiseResult 값
    .then(result => {
...
function getCompleteCount(){
    ...
    //두 번째 then 의 result 
    // - 첫 번쨰 then 에서 text로 변환된 응답 데이터(completeCount값)
    .then(result => {

        // # completeCount 요소에 냐용으로 result 값 출력
...
addBtn.addEventListener("click" , () => {
    ...
    // 첫 번째 then에서 반환된 값 중 반환된 text를 temp에 저장
    .then(temp => {
        if(temp > 0){ // 성공
            alert("추가 성공");
