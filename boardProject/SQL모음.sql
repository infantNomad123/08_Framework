...
UPDATE "MEMBER"
SET MEMBER_ADDRESS = '01810^^^서울 노원구 하계동 78^^^'
WHERE MEMBER_NO = '1';

UPDATE "MEMBER"
SET MEMBER_ADDRESS = 'QQQ^^^QQQ^^^QQQ'
WHERE MEMBER_NO = '1';

UPDATE "MEMBER"
SET MEMBER_ADDRESS = 'HHH^^^HHH^^^HHH'
WHERE MEMBER_NO = '10';
...
CREATE TABLE "MEMBER" (
	...
	"MEMBER_EMAIL"	NVARCHAR2(50)		NOT NULL,
	"MEMBER_PW"	NVARCHAR2(100)		NOT NULL,
	"MEMBER_NICKNAME"	NVARCHAR2		NULL,
	"MEMBER_TEL"	CHAR(11)		NOT NULL,
...
COMMENT ON COLUMN "UPLOAD_FILE"."FILE_RENAME" IS '파일 변경병';
...
--댓글 삽입 확인
SELECT BOARD_NO,COUNT(*) FROM "COMMENT"
GROUP BY BOARD_NO ORDER BY BOARD_NO;
...
--지정된 게시판 (boardCode)에서 삭제된지 않은 게시글 수를 조회
SELECT COUNT(*)
FROM "BOARD"
WHERE BOARD_DEL_FL = 'N'
AND BOARD_CODE = 3;
...
SELECT * FROM BOOK;
CREATE SEQUENCE BOOK_NO NOCACHE;
...
/*좋아요 테이블 (BOARD_LIKE) 샘플 대이터 추가*/
INSERT INTO "BOARD_LIKE"
VALUES(1,1997);  -- 1번 회원이 1998본 글에 조호아요를 클릭함
...
-- 시퀀스로 번호 생헝하는 부분을 별도 함수로 분리 후 호출하면 문제 없음
...
-- SEQ_IMG_NO 시퀀스의 다음 값을 바나환하는 함수 생성
...
SELECT * FROM "MEMBER";
SELECT COUNT(*) FROM "MEMBER"
WHERE MEMBER_EMAIL = 'jkhwan59@gmail.com';
