...
@Transactional // 해당 클래스 메서드 종료 시 까지
					   // - 예외가 발생하지 않으면 commit
					   // - 중간에 예외 (RuntimeException)이 발생하지 않으면 commit
 ...
 *  
 *  ex) 1234 입력        -> $12!asdfg
 *  ex) 1234 다시 입력 -> $12!qwexz
 *  
 *  -비밀번호 확인 방법
 *   -> BCryptPasswordEncoder.matches(평문 비밀번호, 암호회된 비밀번호)
 *   		-> 평문 비밀번호와  암호화된 비밀번호가 같은 경우 true
 *   				아니면 false 반환
 *   
 *   로그인/비밀번호 변경/탈퇴 등 비밀번호가 입력되는 경우 
 *   DB에 저장된 암호화된 비밀번호를 조회해서 matches() 메서드로 비교해야 한다!!
 */
