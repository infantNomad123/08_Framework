...
/*Spring Scheduler : 스프링에서 제공하는 일정  시긴 / 주기 마다 예정된 코드를 실행하는 객체
 * 
 * [설정 방법]
 * 1. 프로젝트명 Application.java 파일에 
 * 		@EnableScheduling 어노테이션 추가
 * 
 * 2. 스케쥴러 코드를 작성할 별도 클래스를 생성한 후 Bean 으로 등록
 * 		-> @Component 어노테이션 작성
 * 
 * 3. 해당 클레스에 @Scheduled(시간/주기) 어노테이션을 추라한 메서드 작성
 * 		*주의사항*
 * 		- 해당 메소드는 반환형이 존재해서는 안된다!! == void
 * 
 * 
 * 
 */

@Component // bean 등록 -> 지동으로 스프링이 자동으로 스케쥴링 코드를 수행하게 함 
@Slf4j
public class TestScheduling {
	...
	// 1) fixedDelay : 이전 작업이 끝난 후 다음 작업이 시작 할 때 까지의 사간을 지정
	
	// 2) fixedRate  : 이전 작업이 시작한 후 다음 작업이 시작 할 때 까지의 사간을 지정
