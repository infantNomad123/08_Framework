...
public class MyPageServiceImpl implements MyPageService{
	...
	private String profileFolderPath;    // C:/
...
//	}
	...
	@Override
	public int updateYN(int memberNo, String memberPw) {
		
		//현재 로그인한 회원의 암호화된 비밀번호를 DB에서 조회
		String pw = mapper.changeYN(memberNo);
		
		
		String encPw = bcrypt.encode(memberPw);
		if(!bcrypt.matches(pw, encPw)) { //다를 경우
			return 0;
		}
		
		return mapper.secessYN(memberNo);
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws IllegalStateException, IOException {
		...
		// 장점 2) method chaining을 이용해서 한줄로 작성이 가능
