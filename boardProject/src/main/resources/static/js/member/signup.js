...
//필수 입룍 항목 유효성 검사 여부를  체크하기위한 객체(체크리스트)
// - true == 해덩 항목은 유효한 형식으로 작성됨
...
memberPw.addEventListener("input" , e => {
    ...
    if(inputPw.trim().length === 0){
        ...
        memberPw.value = ""; //처음 뛰어쓰기 입력 못하게 하기
        return;
    }
...
memberNickname.addEventListener("input", e => {
    ...
    if(inputNick.trim().length === 0){
        ...
        nickMessage.classList.remove("confirm", "error"); // 검은 글씨
        checkObj.memberNickname = false; // 비밀번호가 유효하지 않다고 표시
        memberNickname.value = ""; //처음 뛰어쓰기 입력 못하게 하기
        return;
    }
     const regExp = /^[a-zA-Z0-9!@#_-]{2,10}$/;

    if(!regExp.test(inputNick)){ // 유효하지 않으면
    ...
    .then( count => {
        if(count == 1){
            ...
            nickMessage.classList.add("error");
            nickMessage.classList.remove("confirm");
            checkObj.memeberNickname = false;
            return;
        }
    })
...
memberTel.addEventListener("input", e => {
   ...
   if(inputTel.trim().length === 0){
    ...
    if(!regExp.test(inputTel)){
        telMessage.innerText = "유효하지 않는 번호거나 '-' 를 제외해주세요";
        telMessage.classList.add("error");
        telMessage.classList.remove("confirm");
        checkObj.memberTel = false;
        return;
    }
    telMessage.innerText = "알맞는 형식입니다";
