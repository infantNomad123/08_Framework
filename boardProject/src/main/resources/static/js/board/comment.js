...
addContent.addEventListener("click" , e => {
    ...
    const data = {
        ...
        "boardNo"        : boardNo,
        "memberNo"       : loginMemberNo // 또는 Session 회원 반호 이용도 가능   
    };
...
const insertCancel= (cancelBtn) => {
  ...
  const deleteComment = commentNo => {
    ...
    .then( result => {  
        if(result > 0) {
            alert("삭제 되었습니다");
            selectCommentList(); //다시 조회해서 다시 만들기
        }else{
            alert("삭제 실패");
        }
    })
