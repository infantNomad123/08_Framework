...
if(changePw != null){
  ...
  changePw.addEventListener("submit" , e => {
    ...
    if(!regExp.test(newPw.value)){ // 새비밀번호 정규식 통과 x
      alert("새 비밀번호가 올바른 형식이 아닙니다")
      e.preventDefault();
      return;
...
//프로필 이미지가 새로 업로드 되거나 삭제 되었으을 기록하는 
// 상태 변수
...
// input type="file" 태그가의 값이 변경 되었을 떄
// 변경된 상태를 백업해서 저장할 변수
...
const changeImageFn = e => {
  ...
  if(file == undefined){
    ...
    //화면데 존재하는 기존 input 제거
    imageInput.remove();


    //imageInput 변수에 뱍업을 대입해서 대신하도록 함
    ...
    //backupInput의 백업본이 temp를 backupInpu으로 변경
    backupInput = temp;
  ...
  if(file.size > maxSize){
    ...
    //선택한 아미지가 없는데 5MB 초과하는 이미지를 선택한 경우
    if(statusCheck == -1){
      imageInput.value = '';
...
3. 선택된 파일 정보를 저장하는 속성은 value가 아니라 files 이다
*/